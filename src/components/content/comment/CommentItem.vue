  ...
  <div class="coment-item">
    <div class="base-info">
      ...
      <div class="right-info">
        ...
        <div
          class="be-replied"
          v-if="item.beReplied != null && item.beReplied.length != 0"
        >
          <div v-for="(item2, index2) in item.beReplied" :key="index2">
            <p>
              <span @click="toUserDetail(item2.user.userId)"
    ...
    <div class="like">
      <span
        ...
        :class="{ isLike: isLike }"
        @click="isLikeCommentBy(item)"
        >{{  isLike ? item.likedCount + 1 : item.likedCount |formatNum }}</span
      >
...
export default {
  ...
  created() {
    // 判断是否点赞
    this.cidList = localStorage.getItem("likeComment")
      ? JSON.parse(localStorage.getItem("likeComment"))
      : [];
